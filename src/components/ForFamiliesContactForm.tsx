...
export default function ForFamiliesContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/families-lead", {
        method: "POST",
  ...
  if (submitted) {
    return (
      <div className="bg-white rounded-xl p-8 max-w-lg mx-auto shadow-md text-center">
        ...
        <p className="text-gray-500 text-sm leading-relaxed">
          Thanks, {form.name}. Your inquiry has been personally received and we will be in touch shortly.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl p-6 max-w-lg mx-auto shadow-md">
      ...
      <p className="text-gray-400 text-sm text-center mb-5">
        Personally reviewed. No automated responses.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-navy-800 mb-1">
            Tell us about the property situation <span className="text-gray-400 font-normal">(optional)</span>
          </label>
          <textarea
            name="message"
            value={form.message}
            onChange={handleChange}
            rows={4}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-navy-500 resize-none"
            placeholder="Include property address, current situation, or any questions you have…"
          />
        </div>
